...
public class Problem037 {
	...
	private static LongStream truncations(String target) {
		...
		for (int i = 1; i < target.length(); i++) {
			builder.add(target.substring(i))
					.add(target.substring(0, target.length() - i));
		}
	...
	public static void main(String[] args) {
		System.out
				.println(
						Utils
								.getPrimes(p -> p > 20
										&& (p.toString().endsWith("3")
												|| p.toString().endsWith("7"))
										&& (p == 23 || onlyOdds(p.toString()))
										&& truncations(p.toString())
												.allMatch(tr -> Utils.isPrime(tr)))
