...
public class Problem080 {
    ...
    static BigInteger squareRootDecimals(BigInteger target) {
        BigInteger remainder = target; // p
        BigInteger result = BigInteger.ZERO; // c
        BigInteger x = target.sqrt();
        BigInteger y = x.multiply(result.multiply(BigInteger.valueOf(20).add(x)));
        result = result.multiply(BigInteger.valueOf(10).add(x));
        while (result.toString().length() < 100) {
            remainder = remainder.multiply(BigInteger.valueOf(100));
        }
        return result;
    }
