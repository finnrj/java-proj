...
/**
 * </div>
 * <h2>Lychrel numbers</h2> <div id="problem_info" class="info">
 * <h3>Problem 55</h3> <span>Published on Friday, 24th October 2003, 06:00 pm;
 * Solved by 35268; Difficulty rating: 5%</span> </div>
...
public class Problem055 {
	...
	private static BigInteger reverse(BigInteger target) {
		return new BigInteger(
				new StringBuilder(target.toString()).reverse().toString());
	}

	public static boolean isLychrelNumber(BigInteger target) {
		for (int i = 0; i < 50; i++) {
			target = target.add(reverse(target));
			if (Utils.isPalindrome(target.toString())) {
				return false;
			}
		}
		return true;
	...
	public static void main(String[] args) {
		System.out.println(IntStream.range(1, 10000)
				...
				.filter(Problem055::isLychrelNumber).peek(System.out::println).count());
	}
}
